...
public class QuotationRequestImpl extends PdfGenerator {
	
	public QuotationRequestImpl(BuySaleTaskReq taskReq) {
		super(taskReq);
		
		List<Phrase> docNames = new ArrayList<Phrase>();
		
		Phrase docName = new Phrase();
		docName.add(new Chunk("ใบสอบถามราคา\nQUOTATION REQUEST", angsaBold14));
	...
	protected PdfPTable partnerInfo(BuySaleTaskReq taskReq) throws Exception {
    	...
    	try {
            ...
            pdfPTable.setWidths(new float[]{0.4f, 0.6f, 0.2f, 0.5f, 0.2f, 0.5f});
            ...
            ch = new PdfPCell(new Phrase("ขอสอบถามราคาโดย  : ", angsa14));
            ch.setHorizontalAlignment(Element.ALIGN_RIGHT);
            ch.setBorder(Rectangle.NO_BORDER);
            pdfPTable.addCell(ch);
